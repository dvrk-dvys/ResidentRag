...
def get_qdrant_ids(query, limit=50):
    ...
    #!!! speed up
    vec = EMBED_MODEL.encode([query], normalize_embeddings=True, batch_size=32, convert_to_numpy=True)[0]
...
def medical_query_conditional_boost(q: str, size: int = 50, source_filter: Optional[str] = None,   ) -> Dict:
    """
...
def get_es_ids(query: str, size: int = 50, source_type: Optional[str] = None) -> List[str]:
...
def weighted_rrf_fuse(lists, weights=None, k=60, top_k=10):
    ...
    if weights is None: weights = [1.0] * len(lists)
    scores = {}
